...
app.get("/create", async(req, res) => {
  if (req.isAuthenticated()) {
    ...
    res.render("create.ejs", {profileData: profileData});
  } else {
      res.redirect("/login");
  }
});
...
app.get("/profile", async(req, res) => {
    if (req.isAuthenticated()) {
      ...
      profileData.dob= d;
      res.render("profile.ejs", { profileData: profileData });
    } else {
        res.redirect("/login");
    }
  });
...
app.post("/submit", async(req, res) => {
    if (req.isAuthenticated()) {
      const user_id = req.user.id;
      await db.query("UPDATE profile SET fname=($1), lname=($2), dob=($3), number=($4), details=($5), bio=($6) WHERE id =($7)",[req.body.firstName, req.body.lastName, req.body.dob, req.body.mobileNumber, req.body.educationDetails, req.body.bio, user_id]);
      const result = await db.query("SELECT * FROM profile WHERE id=($1)",[user_id]);
